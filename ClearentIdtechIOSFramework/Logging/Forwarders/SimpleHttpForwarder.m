...
static NSString *const LINE_DELIMITER = @"dlmt";
...
-(ClearentLoggingRequest*) parseData:(NSData*) responseData
{
        ...
        for (NSString *line in logLines) {
            if(![line isEqualToString:@""]) {
                ...
                if(logData != nil) {
                    ...
                    if(clearentLogging.level == nil) {
                        clearentLogging.level = @"info";
                    }
                    clearentLogging.message = logData[1];
                    if(clearentLogging.message == nil) {
                        clearentLogging.message = @"EMPTY";
                    }
                    clearentLogging.createdDate = logData[2];
                    if(clearentLogging.createdDate == nil) {
                        clearentLogging.createdDate = @"nodate";
                    }
    ...
    @catch (NSException *e) {
           [TeleportUtils teleportDebug:@"Failed to parse log."];
    }
              ...
              completion:(void (^)(BOOL success, NSString *errorMessage))completion
{
    ...
    if (serializationError) {
        [TeleportUtils teleportDebug:[NSString stringWithFormat: @"serializationError"]];
        return;
    }
